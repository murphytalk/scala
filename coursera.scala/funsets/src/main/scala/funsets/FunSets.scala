...
object FunSets {
   ...
   * that satisfies `p`.
   * Hint:at least one meets P equals to not all does not meet P
   */
  def exists(s: Set, p: Int => Boolean): Boolean = !forall(s, x=> !p(x))  

  /**
   * Returns a set transformed by applying `f` to each element of `s`.
   * Hint: x belongs to mapped set if there is a value in the Set s which will be equal to x after applying function f
   */
