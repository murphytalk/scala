...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = {
    def loop(s:TweetSet, acc:Trending):Trending = {
       if(s.isEmpty) acc
       else{
         val t = s.findMin
         loop(s.remove(t),acc+t)         
       } 
    }
    loop(this,new EmptyTrending)
  }
...
object GoogleVsApple {
  ...
  val googleTweets: TweetSet = allTweets.filter( t => google.exists(t.text.contains))

  val appleTweets: TweetSet = allTweets.filter( t => apple.exists(t.text.contains))
